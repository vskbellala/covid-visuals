...
df_us = df.loc[(df['county'] == 'Duval') & (df['state'] == 'Florida')]
...
fig1.add_trace(go.Scatter(x=df_us['date'], y=df_us['cases'],name="Cases", line=dict(color='#fc03a5'))) # pos case graph
fig1.add_trace(go.Scatter(x=df_us['date'], y=df_us['deaths'],name="Deaths", line=dict(color='#7b03fc'))) # death graph
...
link = 'event_lists/jacksonville_sheet.csv'
...
fig1.update_layout(hovermode="x unified", # consistent hover
    ...
    xaxis_title="Date",
    yaxis_title="Count",
    title="COVID-19 Cases & Deaths in Jacksonville, FL",
...
fig1.write_html(file="../../docs/plots/jacksonville_lines.html",auto_play=True,full_html=False,include_plotlyjs='cdn',
