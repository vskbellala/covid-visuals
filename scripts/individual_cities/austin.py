...
df_us = df.loc[(df['county'] == 'Travis') & (df['state'] == 'Texas')]
...
fig1.add_trace(go.Scatter(x=df_us['date'], y=df_us['cases'],name="Cases", line=dict(color='#453ffc'))) # pos case graph
fig1.add_trace(go.Scatter(x=df_us['date'], y=df_us['deaths'],name="Deaths", line=dict(color='#201d80'))) # death graph
...
fig1.update_layout(hovermode="x unified", # consistent hover
    ...
    xaxis_title="Date",
    yaxis_title="Count",
    title="COVID-19 Cases & Deaths in Austin, TX",
