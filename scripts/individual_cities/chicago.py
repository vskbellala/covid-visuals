...
df_us = df.loc[(df['county'] == 'Cook') & (df['state'] == 'Illinois')]
...
fig1.add_trace(go.Scatter(x=df_us['date'], y=df_us['cases'],name="Cases", line=dict(color='#ff0d6a'))) # pos case graph
fig1.add_trace(go.Scatter(x=df_us['date'], y=df_us['deaths'],name="Deaths", line=dict(color='#b3295e'))) # death graph
...
link = 'event_lists/chicago_sheet.csv'
...
fig1.update_layout(hovermode="x unified", # consistent hover
    ...
    xaxis_title="Date",
    yaxis_title="Count",
    title="COVID-19 Cases & Deaths in Chicago, IL",
...
fig1.write_html(file="../../docs/plots/chicago_lines.html",auto_play=True,full_html=False,include_plotlyjs='cdn',
