...
df_us = df.loc[(df['county'] == 'King') & (df['state'] == 'Washington')]
...
fig1.add_trace(go.Scatter(x=df_us['date'], y=df_us['cases'],name="Cases", line=dict(color='#e61c73'))) # pos case graph
fig1.add_trace(go.Scatter(x=df_us['date'], y=df_us['deaths'],name="Deaths", line=dict(color='#8f2196'))) # death graph
...
link = 'event_lists/seattle_sheet.csv'
...
fig1.update_layout(hovermode="x unified", # consistent hover
    ...
    xaxis_title="Date",
    yaxis_title="Count",
    title="COVID-19 Cases & Deaths in Seattle, WA",
...
fig1.write_html(file="../../docs/plots/seattle_lines.html",auto_play=True,full_html=False,include_plotlyjs='cdn',
